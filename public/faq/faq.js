function toggleFAQChild(faqID){
	...
	var allQuestions = document.getElementsByClassName("faqQuestion");
	...
	for(i=0;i<allAns.length;i++){
		...
		if(allAns[i].id != "faqAnswer" + faqID && document.getElementById(allAns[i].id).style.display != "none"){
			...
			document.getElementById(allQuestions[i].id).style.fontWeight="100";
		}
	}
	...
	if(faqAns.style.display == "none"){
		...
		document.getElementById("faqQuestion" + faqID).style.fontWeight="700";
	}
	else{
