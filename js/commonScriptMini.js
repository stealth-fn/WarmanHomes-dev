...
accordionTree.prototype.resNavConstruct = function(){
	...
	if(this.responsiveNavType == 0){
		$('#navOuter-' +this.className).addClass("mobileNav");
	}
	else if(this.responsiveNavType == 1){
		$('#navOuter-' +this.className).addClass("mobileNavExp");
	}
	
	$('#navOuter-' +this.className).addClass("mNav");
	
	//create temporary object from nav object to user in jquery loop
	var tempNavObj = this;
	
	//remove hover events from nav
	$(document).off("mouseenter",".nc");
	$(document).off("mouseleave",'#navOuter-' + this.className + " li");
	$(document).off("mouseout",'#navOuter-' + this.className + " li");
	
	//remove mousein and mouseout events
	$('#navOuter-' + this.className + " li").removeAttr('onmouseout');
	$('#navOuter-' + this.className + " ul").removeAttr('onmouseout');
	
	$('#navOuter-' + this.className + " li").removeAttr('onmouseover');
	$('#navOuter-' + this.className + " ul").removeAttr('onmouseover');
	
	$(document).off("mouseleave",".nc, .ec");
		
	//loop through our links adjust the events as required
	$('#navOuter-' + this.className + " .nc > a.nc").each(
		function(index, el){	
		
			//get the onclick event as a string
			if($(el).attr('onclick')) {
			...
			if($(el).attr('onmouseover')) {
				var tempMouseOver = repSubstr(
					$(el).attr('onmouseover'),"return false",""
				);
			}
			else{
				var tempMouseOver = "";
			...
			//replace our event with null in the parameters
			tempClick = repSubstr(
			...
			if(tempNavObj.responsiveNavType == 0){
				$(el).attr('data-onclick',tempClick);
			}
			else if(tempNavObj.responsiveNavType == 1){
				$(el).attr(
					'data-onclick',
					tempClick + tempMouseOver + ";atpto_tNav.menuNavExpand(atpto_tNav,event)"
				);
			}
...
accordionTree.prototype.resNav = function(){
	if(
		...
		if($('#navOuter-' + this.className + '.mNav').length == 0){
			
			//prepare for responsive nav
			this.resNavConstruct();
		...
		if($('#navOuter-' + this.className + '.mNav').length > 0){
			...
			$(document).on("mouseenter",".nc",{stealthNavObj:this},function(e){
				$(e.data.stealthNavObj).children(".ec").andSelf().addClass("hover");
			});
...
//make hamburger menu expand when nav when clicked
accordionTree.prototype.menuNavExpand = function(stealthNavObj,e){
	
		disabledEventPropagation(e);

		$(".hamburger--" + stealthNavObj.hamburgerAnimation).toggleClass("is-active");
		$('#navOuter-' + stealthNavObj.className).toggleClass("closedMobileNav");
		$('#navOuter-' + stealthNavObj.className + " .navInner").toggleClass("mNavClosed");
		
		//clase all pages trees
		$(".hover").removeClass("hover");
	
}

accordionTree.prototype.resNavExpand = function(){
	if(
		//instance allows responsiveness
		this.isResponsive == true &&
		//screen size is less than the transition point
		(
			(window.matchMedia &&
			window.matchMedia("(max-width: " + this.triggerPoint + "px)").matches) ||
			//older browsers
		...
		//and is isn't already responsive
		$('#navOuter-' + this.className + '.mNav').length === 0
	) {
		//prepare for responsive nav
		this.resNavConstruct();
		
		$("#responsiveBtn-" + this.className).on("click",{stealthNavObj:this},function(e){
			e.data.stealthNavObj.menuNavExpand(e.data.stealthNavObj,e);
		});
		
		$('#navOuter-' + this.className + " .navInner").addClass("mNavClosed");
				
		//loop through our links and attach the onclicks as events
		$('#navOuter-' + this.className + " .hc > .ec").each(
			function(inex, el){
				...
				$("#childShow" + $(el).get(0).id).attr('data-onclick',tempMouseOver);
				
				$("#childShow" + $(el).get(0).id).on('click',
					function(event){
						disabledEventPropagation(event);
					
						//close all expanded navs no under the current root page
						$(".ntl1").not($(this).parents()).removeClass("hover");
						$(".ntl1").not($(this).parents()).find(".hover").removeClass("hover");

						//expand current section
						$(this).parent().toggleClass("hover");
						$(this).prev(".ec").toggleClass("hover");
		...
		//instance allows responsiveness
		this.isResponsive == true &&
		//screen size is greater than the transition point
		(
			(window.matchMedia &&
			!window.matchMedia("(max-width: " + this.triggerPoint + "px)").matches) ||
			//older browsers
...
responsiveNav = function(){
	...
	for(var prop in accordionTreeObjs){	
		...
		if(
			accordionTreeObjs[prop].isResponsive && 
			accordionTreeObjs[prop].responsiveNavType == 0
		)	{
			accordionTreeObjs[prop].resNav(tempProp);
		}
		//expand to show children under parents
		else if(
			accordionTreeObjs[prop].isResponsive && 
			accordionTreeObjs[prop].responsiveNavType == 1
		){
			accordionTreeObjs[prop].resNavExpand(tempProp);
		}
	}
}
...
$(window).load(function(){
	...
	for(var prop in accordionTreeObjs){	
		...
		accordionTreeObjs[prop].hoverInit();
	}
...
stealthCommon.prototype.getCSVDump = function(requestParams){
	window.open("/public/moduleFrame/moduleInstanceSet.php?" + requestParams + "&csvDump=1");
}
...
function editOrder(orderID,pageID) {
	
	var editOrderAddCart = ajaxObj();
	ajaxPost(
		editOrderAddCart,
		"/cmsAPI/ecommerce/cmsCart/cmsCart.php",
		"function=editOrderAddCart&orderID=" + orderID,
		true,
		0,null,false
	);
	
	alert("Order: "+orderID+" is now in Edit Mode. Proceed to checkout to save changes");
	upc(-7);
}
///////////////////////////////////////////////////////////////
